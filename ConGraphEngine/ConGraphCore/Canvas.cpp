...
void Canvas::drawCircle(int x, int y, int radius, char character, COLOR drowColor, COLOR background, bool isFill)
{
	COORD c;
	c.X = x;
	c.Y = y;
	...
	{
		c.X = x + 40;
		c.Y = 12 - static_cast<int>(sqrt(radius * radius - x * x));
		cell(x, y, character, drowColor, background, BaseDrawElement(0, 0, width_, height_));
	...
	BaseDrawElement rect = rect.intersectRect(BaseDrawElement(0, 0, width_, height_));
	int right = rect.getRight();
	int bottom = rect.getBottom();
	for (int i = rect.x; i < right; i++)
	{
		for (int j = rect.y; j < bottom; j++)
		{
			cells_[i][j].set(character, drowColor, background);
		}
	}
}
